...
https://pubchem.ncbi.nlm.nih.gov/rest/pug/.../xref/RegistryID/<id>/... does
over the network (see the old prep_cigs.jl `pubchem_smiles`).
...
same-stereo, etc.) whose exact mapping isn't decoded here. Every
association is captured as-is in the `cid_links` table (see
[`parse_substances`](@ref)) rather than guessing which type code means "the"
canonical CID. If a real build turns out to over-match (e.g. a mixture's
mixture-component associations pulling in unrelated CIDs), filter
`cid_links` by `assoc_type` before building the lookup index, once the type
codes are pinned down.
"""
...
- `substances`: one row per SID, `sid` and `source`.
...
function build_substances_tables(data_dir::AbstractString; out_dir::AbstractString=data_dir,
    ...
    Threads.foreach(ch_blk; ntasks=block_tasks) do blk_i
        ...
        @info "Done with block $blk_i." sub_out link_out id_out rss_mb=rss_mb()
    end
    return nothing
end
